import XCTest
@testable import ThimbleTray

@MainActor
final class ThimbleTrayTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        let item = Thimble(title: "Test Item")
    ...
    func testAddRespectsFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Thimble(title: "Item \(i)"), isPro: false)
        }
        let added = store.add(Thimble(title: "Overflow"), isPro: false)
    ...
    func testProBypassesFreeLimit() {
        for i in 0..<Store.freeLimit {
            _ = store.add(Thimble(title: "Item \(i)"), isPro: true)
        }
        let added = store.add(Thimble(title: "Extra"), isPro: true)
        XCTAssertTrue(added)
        XCTAssertEqual(store.items.count, Store.freeLimit + 1)
    }

    func testDeleteByIdRemovesItem() {
        let item = Thimble(title: "Delete Me")
        _ = store.add(item, isPro: false)
    ...
    func testUpdateChangesTitle() {
        var item = Thimble(title: "Original")
        _ = store.add(item, isPro: false)
        item.title = "Updated"
        store.update(item)
    ...
    func testPersistenceRoundTrip() {
        let item = Thimble(title: "Persisted")
        _ = store.add(item, isPro: false)
        let reloaded = Store()
